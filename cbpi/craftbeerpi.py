...
class CraftBeerPi:
    ...
    def __init__(self, configFolder):
        ...
        self.app = web.Application(
            middlewares=middlewares, client_max_size=5 * 1024 * 1024
        )
        self.app["cbpi"] = self
        ...
        self.fermenterrecipe: FermenterRecipeController = FermenterRecipeController(
            self
        )
    ...
    def push_update(self, topic, data, retain=False) -> None:
        ...
        if self.satellite is not None:
            asyncio.create_task(
                self.satellite.publish(
                    topic=topic, message=json.dumps(data), retain=retain
                )
            )
    ...
    def start(self):
        ...
        if not self.testport(port):
            ...
            logging.error(
                "Port {} is already in use! Please check, if server is already running (e.g. in automode)".format(
                    port
                )
            )
            exit(1)
